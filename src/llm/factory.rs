...
impl LLMFactory {
    ...
    /// ```
    pub fn create(base_llm: Box<dyn LLM>, retry_config: Option<RetryConfig>) -> Box<dyn LLM> {
        match retry_config {
            ...
            Some(_) => {
                tracing::debug!(
                    "Retry config provided but max_attempts is 0, using base LLM without retry"
                );
                base_llm
            }
            None => {
